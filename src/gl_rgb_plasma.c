...
#define VERTEX_SHADER_PATH "src/gl_rgb_plasma.vert"
#define FRAGMENT_SHADER_PATH "src/gl_rgb_plasma.frag"
...
void LogShaderError(GLuint shader) {
  if (glIsShader(shader)) {
    ...
    if (infoLog == NULL) {
      LogError("failed to malloc error log for gl shader %d of size %d", shader,
               maxLength);
    }
...
int InitGL(void) {
  ...
  gUniformResolutionLocation = glGetUniformLocation(gProgramId, "uResolution");
